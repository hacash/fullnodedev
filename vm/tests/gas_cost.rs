...
use vm::rt::{Bytecode, CallExit, ExecMode, GasExtra, GasTable, SpaceCap};
...
impl GasTestContext {
    ...
    fn print_report(&self) {
        if self.mismatches.is_empty() {
            ...
            println!(
                "\n❌ Found {} gas consumption mismatches:",
                self.mismatches.len()
            );
...
fn get_base_gas(opcode: Bytecode) -> i64 {
    use Bytecode::*;
    match opcode {
        PU8 | P0 | P1 | P2 | P3 | PNBUF | PNIL | PTRUE | PFALSE | CU8 | CU16 | CU32 | CU64
        | CU128 | CBUF | CTO | TID | TIS | TNIL | TMAP | TLIST | POP | NOP | NT | END | RET
        | ABT | ERR | AST | PRT => 1,

        BRL | BRS | BRSL | BRSLN | XLG | PUT | CHOOSE => 3,

        DUPN | POPN | PICK | PBUF | PBUFL | MOD | MUL | DIV | XOP | HREAD | HREADU | HREADUL
        | HSLICE | HGROW | ITEMGET | HEAD | BACK | HASKEY | LENGTH => 4,

        POW => 5,

        HWRITE | HWRITEX | HWRITEXL | INSERT | REMOVE | CLEAR | APPEND => 6,

        CAT | BYTE | CUT | LEFT | RIGHT | LDROP | RDROP | MGET | JOIN | REV | NEWLIST | NEWMAP
        | NTFUNC => 8,

        EXTENV | NTENV | MPUT | CALLTHIS | CALLSELF | CALLSUPER | PACKLIST | PACKMAP | UPLIST
        | CLONE | MERGE | KEYS | VALUES => 12,
...
fn test_stack_copy_gas(ctx: &mut GasTestContext, opcode: Bytecode, buffer_size: usize) {
    ...
    run_gas_test(
        ...
        codes,
        expected,
        &format!("Stack copy ({} bytes)", buffer_size),
        opcode,
        resource_sizes,
    );
}
...
fn test_heap_grow_gas(ctx: &mut GasTestContext, seg_count: u8) {
    ...
    run_gas_test(
        ...
        codes,
        expected,
        &format!("Heap grow ({} segments)", seg_count),
        Bytecode::HGROW,
        resource_sizes,
    );
}
...
fn test_heap_read_gas(ctx: &mut GasTestContext, read_length: usize) {
    ...
    run_gas_test(
        ...
        codes,
        expected,
        &format!("Heap read ({} bytes)", read_length),
        Bytecode::HREAD,
        resource_sizes,
    );
}
...
fn test_heap_write_gas(ctx: &mut GasTestContext, write_length: usize) {
    ...
    run_gas_test(
        ...
        codes,
        expected,
        &format!("Heap write ({} bytes)", write_length),
        Bytecode::HWRITE,
        resource_sizes,
    );
}
    ...
    let total = match execute_test_with_argv(gas_limit, total_codes, None) {
        ...
        Err(e) => {
            ctx.record_mismatch(
                Bytecode::SLOAD,
                format!(
                    "Storage load ({} bytes, execute failed: {:?})",
                    value_size, e
                ),
                expected,
                -1,
...
fn test_storage_save_gas(ctx: &mut GasTestContext, value_size: usize, is_new_key: bool) {
    ...
    let total = match execute_test_with_argv(gas_limit, total_codes, None) {
        ...
        Err(e) => {
            ctx.record_mismatch(
                Bytecode::SSAVE,
                format!(
                    "Storage save ({} bytes, execute failed: {:?})",
                    value_size, e
                ),
                expected,
                -1,
    ...
    if actual != expected {
        ctx.record_mismatch(
            Bytecode::SSAVE,
            format!(
                "Storage save ({} bytes, new_key={})",
                value_size, is_new_key
            ),
            expected,
            actual,
            resource_sizes,
...
fn test_compo_gas(ctx: &mut GasTestContext, opcode: Bytecode, item_count: usize, byte_size: usize) {
    ...
    run_gas_test(
        ...
        codes,
        expected,
        &format!("Compo op (items={}, bytes={})", item_count, byte_size),
        opcode,
        resource_sizes,
    );
}
...
fn test_space_alloc_gas(ctx: &mut GasTestContext, opcode: Bytecode, count: usize) {
    ...
    run_gas_test(
        ...
        codes,
        expected,
        &format!("Space alloc (count={})", count),
        opcode,
        resource_sizes,
    );
}
...
mod tests {
    ...
    fn test_all_base_gas() {
        ...
        for opcode in gas3_opcodes {
            let setup = match opcode {
                ...
                Bytecode::CHOOSE => {
                    vec![Bytecode::P1 as u8, Bytecode::P0 as u8, Bytecode::P1 as u8]
                }
    ...
    fn test_space_alloc_gas() {
        ...
        let alloc_opcodes = vec![Bytecode::ALLOC, Bytecode::MPUT, Bytecode::GPUT];
