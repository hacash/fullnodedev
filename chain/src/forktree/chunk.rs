...
impl ChunkRef {
    ...
    pub(super) fn children(&self) -> Vec<ChunkRef> {
        self.0.childs.read().unwrap().iter().cloned().map(ChunkRef).collect()
    }
    ...
    pub(super) fn append(&self, child: &ChunkRef, fast_sync: bool) -> Rerr {
        ...
        if child.0.height != self.0.height + 1 {
            return errf!("child height need {} but got {}", self.0.height + 1, child.0.height);
        }
        let mut childs = self.0.childs.write().unwrap();
        if childs.iter().any(|c| Arc::ptr_eq(c, &child.0) || c.hash == child.0.hash) {
