impl DifficultyGnr {
    pub(crate) fn req_cycle_block(&self, hei: u64, src: &dyn BlockIntroSource) -> CachedBlockIntro {
        let cylnum = self.cnf.difficulty_adjust_blocks;
        if hei < cylnum {
            return self.req_block_intro(0, src)
        }
        let cylhei = hei / cylnum * cylnum;
        self.req_block_intro(cylhei, src)
    }
    ...
    fn target_legacy(
        ...
        let (prevcltime, _, _) = self.req_cycle_block(hei - cylnum, src);
