...
2. `BUG 2 | L | vm/src/frame/call.rs:80-100`
   `CODECALL` 复用当前帧且不清空 operand stack / locals / heap，属于既定委托执行模型，允许被调代码访问调用者帧内局部上下文。

3. `BUG 3 | L | vm/src/frame/call.rs:80-82`
   `CODECALL` 路径直接使用 `plan.code_owner()` 且不额外 `assert KeepState`，属于调用计划构建阶段已保证的不变量约束。

4. `BUG 4 | L | vm/src/frame/frame.rs:121 + vm/src/frame/call.rs:96`
   `ret_check_policy` 的默认赋值后再由 `CODECALL` 分支覆写，属于允许的冗余赋值路径，不构成行为错误。

5. `BUG 5 | L | vm/src/interpreter/execute.rs:256-258`
   `CODECALL` 上下文仅禁止 `EXTACTION`，放行 `EXTENV` 与 `EXTVIEW` 属于既定外部交互分层策略。
