...
action_define!{AssetToTrs, 17, 
    ...
    (self, format!("Transfer {} to {}", self.asset, self.to.readable())),
    (self, ctx, _gas {
...
action_define!{AssetFromTrs, 18, 
    ...
    (self, format!("Transfer {} from {}", self.asset, self.from.readable())),
    (self, ctx, _gas {
...
action_define!{AssetFromToTrs, 19, 
    ...
    (self, format!("Transfer {} from {} to {}", self.asset, self.from.readable(), self.to.readable())),
    (self, ctx, _gas {
        let from = ctx.addr(&self.from)?;
        let to   = ctx.addr(&self.to)?;
        asset_transfer(ctx, &from, &to, &self.asset)
