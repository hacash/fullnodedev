...
pub fn analyze_test_results() {
    ...
    println!(
        "- Gas cost = 1: {} opcodes",
        expected_base_gas.values().filter(|&&v| v == 1).count()
    );
    println!(
        "- Gas cost = 2: {} opcodes",
        expected_base_gas.values().filter(|&&v| v == 2).count()
    );
    println!(
        "- Gas cost = 3: {} opcodes",
        expected_base_gas.values().filter(|&&v| v == 3).count()
    );
    println!(
        "- Gas cost = 4: {} opcodes",
        expected_base_gas.values().filter(|&&v| v == 4).count()
    );
    println!(
        "- Gas cost = 5: {} opcodes",
        expected_base_gas.values().filter(|&&v| v == 5).count()
    );
    println!(
        "- Gas cost = 6: {} opcodes",
        expected_base_gas.values().filter(|&&v| v == 6).count()
    );
    println!(
        "- Gas cost = 8: {} opcodes",
        expected_base_gas.values().filter(|&&v| v == 8).count()
    );
    println!(
        "- Gas cost = 12: {} opcodes",
        expected_base_gas.values().filter(|&&v| v == 12).count()
    );
    println!(
        "- Gas cost = 16: {} opcodes",
        expected_base_gas.values().filter(|&&v| v == 16).count()
    );
    println!(
        "- Gas cost = 20: {} opcodes",
        expected_base_gas.values().filter(|&&v| v == 20).count()
    );
    println!(
        "- Gas cost = 24: {} opcodes",
        expected_base_gas.values().filter(|&&v| v == 24).count()
    );
    println!(
        "- Gas cost = 28: {} opcodes",
        expected_base_gas.values().filter(|&&v| v == 28).count()
    );
    println!(
        "- Gas cost = 32: {} opcodes",
        expected_base_gas.values().filter(|&&v| v == 32).count()
    );
    println!(
        "- Gas cost = 64: {} opcodes",
        expected_base_gas.values().filter(|&&v| v == 64).count()
    );
    println!();
    ...
    println!(
        "   - Possible cause: HGROW base gas might be 5 instead of 4, or there's an extra gas charge\n"
    );
    ...
    println!(
        "   - Possible cause: HGROW(1 segment) costs 10 gas, which is included in HREAD test\n"
    );
