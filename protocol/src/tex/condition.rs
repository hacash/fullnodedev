macro_rules! define_cell_cond_zhu {
    ( $cid: expr, $class: ident, $check_op: ident) => {
        combi_struct! { $class,
            cellid: Uint1
            haczhu: Fold64
        }
        ...
        impl CellExec for $class {
            fn execute(&self, ctx: &mut dyn Context, taradr: &Address) -> Rerr {
                ...
                // TEX conditions intentionally use text errors so failure stays a hard TEX abort instead of a recoverable revert.
...
macro_rules! define_cell_cond_sat {
    ( $cid: expr, $class: ident, $check_op: ident) => {
        combi_struct! { $class,
            cellid: Uint1
            satoshi: Fold64
        ...
        impl CellExec for $class {
            fn execute(&self, ctx: &mut dyn Context, taradr: &Address) -> Rerr {
                let sat = CoreState::wrap(ctx.state())
                    .balance(taradr)
                    .unwrap_or_default()
                    .satoshi
                    .uint();
...
macro_rules! define_cell_cond_dia {
    ( $cid: expr, $class: ident, $check_op: ident) => {
        combi_struct! { $class,
            cellid: Uint1
            diamond: Fold64
        ...
        impl CellExec for $class {
            fn execute(&self, ctx: &mut dyn Context, taradr: &Address) -> Rerr {
                let dia = CoreState::wrap(ctx.state())
                    .balance(taradr)
                    .unwrap_or_default()
                    .diamond
                    .uint();
...
macro_rules! define_cell_cond_asset {
    ( $cid: expr, $class: ident, $check_op: ident) => {
        combi_struct! { $class,
            cellid: Uint1
            asset:  AssetAmt
...
macro_rules! define_cell_cond_height {
    ( $cid: expr, $class: ident, $check_op: ident) => {
        combi_struct! { $class,
            cellid: Uint1
            height: BlockHeight
...
impl TexCell for CellCondChainIdEq {
    fn kind(&self) -> u16 {
        Self::CID as u16
    }
}
