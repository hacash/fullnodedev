...
mod astact {
    ...
    fn t_if() {
        ...
        let cond =
            AstSelect::create_list(vec![Box::new(HacFromTrs::create_by(adr2, Amount::mei(5)))]);

        let br_if = AstSelect::create_list(vec![Box::new(SatToTrs::create_by(
            adr2,
            Satoshi::from(100),
        ))]);

        let br_else =
            AstSelect::create_list(vec![Box::new(HacToTrs::create_by(adr2, Amount::mei(20)))]);
