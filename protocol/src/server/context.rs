...
impl ApiCtx {
    ...
    pub fn load_block(&self, store: &dyn Store, key: &String) -> Ret<Arc<BlkPkg>> {
        self.load_block_from_cache(store, key, true)
    }

    
    // load block from cache or disk, key = height or hash
    pub fn load_block_from_cache(&self, store: &dyn Store, key: &String, with_cache: bool) -> Ret<Arc<BlkPkg>> {
        ...
        let Ok(blkpkg) = build_block_package(blkdts.unwrap()) else {
